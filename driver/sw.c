...
define and const
***********************************************************************************************************************/
#define SW_PUSH_TIME_THRESHOLD_MS	100u//この指定時間[ms]連続でスイッチがONであればソフトウェアとしてpushされたと判定する
#define SW_NON_PUSH					0xFFFFu

const uint EXE_SW_PIN	= 2;
const uint NEXT_SW_PIN	= 3;
const uint BACK_SW_PIN	= 4;
...
void init_sw(void){
	...
	//ピン設定
	gpio_init(BACK_SW_PIN);
	gpio_set_dir(BACK_SW_PIN, GPIO_IN);//回路でプルアップされてないので内臓プルアップ利用
	gpio_pull_up(BACK_SW_PIN);
...
void update_sw(void){
	...
	sw_port_status[SW_BACK] = gpio_get(BACK_SW_PIN);
